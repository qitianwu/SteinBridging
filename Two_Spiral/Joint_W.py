...
# model parameter
lr_d = 1e-5  # learning rate for Wasserstein critic
...
h_dim_d = 128  # number of hidden neurons per layer of Wasserstein critic
h_dim_p = 128 # number of hidden neurons per layer of density estimator
z_dim = 4  # noise dimension
X_dim = 2 # sample dimension
n_critic = 5 # epoches for critic before update for generator in WGAN
lambda_p = 100 # weight for penalizing gradient norm in WGAN
exp_num = 4 # expert num in energy model

mb_size = 100  # mini-batch size
n_iter = 500000 # total interations for training
...
optimizer_WGAN = tf.train.AdamOptimizer # optimizer for Wasserstein loss
...
def generator(z, isTraining):
    with tf.variable_scope("g", reuse=tf.AUTO_REUSE):
        o1 = tf.layers.dense(z, h_dim_g, activation=tf.nn.leaky_relu, name='l1')
        o2 = tf.layers.dense(o1, h_dim_g, activation=tf.nn.leaky_relu, name='l2')
...
def discriminator(x):
    with tf.variable_scope("d", reuse=tf.AUTO_REUSE):
        ...
        out = tf.layers.dense(o2, 1, activation=None, name='l3')

    return out #[mb, 1]

def S_q(xs):
    return tf.gradients(p_theta(xs), xs)[0] #[mx, X_dim]
...
def dis_loss(x_gen, x_true):
    ...
    u = tf.diag(u)
    x_ = tf.matmul(u, x_gen) + tf.matmul(1-u, x_true)
    gradient_norm = tf.sqrt(tf.reduce_sum(
        tf.square(tf.gradients(discriminator(x_), x_)[0]), axis=1))
    penalty = tf.nn.relu(gradient_norm - 1)
    loss = tf.reduce_mean(discriminator(x_gen) - discriminator(x_true) + lambda_p*penalty)
    return loss

G_sample = generator(z, isTraining)
D_loss = dis_loss(G_sample, X_true)
G_loss = -tf.reduce_mean(discriminator(G_sample), 0)
...
solver_WGAN_D = optimizer_WGAN(learning_rate=lr_g).minimize(D_loss, var_list=para_D)

solver_WGAN_G = optimizer_WGAN(learning_rate=lr_g).minimize(G_loss, var_list=para_G)
...
for it in range(load_it, load_it+n_iter+1):
    ...
    while i_critic<=n_critic:
        _, loss_d = sess.run([solver_WGAN_D, D_loss],
                        feed_dict={z: sample_z(mb_size, z_dim), 
                                X_true: datainput.get_batch(),
                                isTraining: True})
        i_critic += 1
    
    _, loss_g = sess.run([solver_WGAN_G, G_loss],
                            feed_dict={z: sample_z(mb_size, z_dim),
                                    isTraining: True})
    ...
    if it % iter_display == 0:
        ...
        ce_y, ent_yx, high_rate = ce_y_eval(fake_sample, 'Two_Spiral')
